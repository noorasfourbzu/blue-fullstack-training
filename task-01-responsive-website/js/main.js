...
const categoryButtonContainer = document.querySelector(
...
const postsStatus =
  document.querySelector(".posts-status"); /* loading,error,empty*/
...
function validateName() {
  ...
  if (name.length < NAME_MIN_LENGTH) {
    showFieldError(
      nameInput,
      nameError,
      `full name must contain at least ${NAME_MIN_LENGTH} characters`,
    );
    return false;
  }
  if (name.length > NAME_MAX_LENGTH) {
    showFieldError(
      nameInput,
      nameError,
      `full name must not exceed ${NAME_MAX_LENGTH} characters`,
    );
    return false;
  }
...
function validatePhone() {
  ...
  if (digitCount < PHONE_MIN_DIGITS || digitCount > PHONE_MAX_DIGITS) {
    showFieldError(
      phoneInput,
      phoneError,
      `phone number must contain between ${PHONE_MIN_DIGITS} and ${PHONE_MAX_DIGITS} digits`,
    );
    return false;
  }
  clearFieldError(phoneInput, phoneError);
...
function initContactForm() {
  ...
  if (contactForm) {
    contactForm.addEventListener("submit", function (event) {
      ...
      formStatus.textContent = "Thank you! Your message was sent successfuly";
...
categoryCapsule.forEach((button) => {
  if (button.textContent === selected) button.classList.add("is-active");
...
function showEmptyState() {
  ...
  postsStatus.textContent =
    "No posts available at the moment, come back later.";
  postsStatus.className = "posts-status";
  resultsCount.textContent = "";
}
...
function loadMorePosts() {
  ...
  const nextBatch = postsArray.slice(
    currentIndex,
