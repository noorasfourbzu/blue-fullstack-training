...
onMounted(() => {
  if (store.posts.length === 0) store.fetchPosts();
});
</script>
  ...
  <section id="favorites" class="section">
    <div class="container">
      ...
      <p v-else-if="store.error" class="posts-status posts-status--error">
        Something went wrong loading your favorites, click<button
          type="button"
      ...
      <p
        v-else-if="store.favoritePosts.length === 0"
      ...
      <template v-else>
        <p class="results-count" aria-live="polite">
          {{ store.favoritePosts.length }} favorite{{
            store.favoritePosts.length === 1 ? "" : "s"
          }}
        </p>
